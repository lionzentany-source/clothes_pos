-- Migration 001: Base schema extracted from original schema.sql for new installs
-- Keeping alignment with legacy schema so later migrations (003/004 etc.) apply correctly.
-- Safe to re-run due to IF NOT EXISTS & UNIQUE constraints.
...
-- Returns (may be complemented by later migrations)
...
-- Settings & audit (legacy audit_log naming kept)
